...
```bash
# 构建镜像
docker build -t universal-agent-mvp .
...
# 克隆代码
git clone https://github.com/your-repo/universal-agent-mvp.git
cd universal-agent-mvp
...
User=agent
Group=agent
WorkingDirectory=/home/agent/universal-agent-mvp
Environment=PATH=/home/agent/universal-agent-mvp/venv/bin
ExecStart=/home/agent/universal-agent-mvp/venv/bin/python -m scripts.start
...
/home/agent/universal-agent-mvp/logs/*.log {
    daily
    missingok
    rotate 7
    compress
...
```json
{
    ...
    "containerDefinitions": [
        {
            "name": "agent",
            "image": "your-registry/universal-agent-mvp:latest",
            "essential": true,
            "portMappings": [
                {
                    "containerPort": 8080,
...
spec:
  template:
    spec:
      containers:
      - image: gcr.io/your-project/universal-agent-mvp:latest
        ports:
        - containerPort: 8080
        env:
        - name: ENVIRONMENT
...
properties:
  ...
  template:
    containers:
    - image: your-registry.azurecr.io/universal-agent-mvp:latest
      name: agent
      env:
      - name: ENVIRONMENT
        value: production
