...
#### 0. 进入项目目录
```bash
cd agent-mvp  # 重要：所有命令都需在项目根目录运行
```
...
```bash
# 构建镜像
docker build -f docker/Dockerfile -t universal-agent-mvp .
...
# 进入容器
docker exec -it universal-agent-mvp bash

# 查看容器日志
docker logs -f universal-agent-mvp

# 清理容器
docker stop universal-agent-mvp
docker rm universal-agent-mvp
```
