...
def session_management_demo():
    ...
    for i, session in enumerate(sessions):
        ...
        # 获取会话历史
        history = requests.get(f"{BASE_URL}/sessions/{session['session_id']}/history").json()
...
async def batch_processing_demo():
    ...
    # 创建会话
    async with aiohttp.ClientSession() as session:
        async with session.post("http://localhost:8080/sessions") as response:
...
func main() {
    // 创建会话
    resp, err := http.Post(BASE_URL+"/sessions", "application/json", bytes.NewBufferString("{}"))
    if err != nil {
        panic(err)
    }
    ...
    resp, err = http.Post(BASE_URL+"/chat", "application/json", bytes.NewBuffer(reqBody))
    if err != nil {
        panic(err)
    }
    defer resp.Body.Close()
...
response = session.post("http://localhost:8080/chat", json={
