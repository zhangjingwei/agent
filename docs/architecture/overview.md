...
🌐 客户端请求
    ↓ HTTP/HTTPS
🏗️ Go网关 (nexus-gateway)
    ↓ (路由 + 会话管理)
    ├─► 缓存检查 (Redis)
    ├─► 限流控制 (Rate Limiting)
    ├─► 请求验证 (Validation)
    ↓
🤖 Python推理服务 (nexus-agent)
    ↓ (内部API调用)
🎯 核心层 (UniversalAgent)
    ↓ (业务编排)
...
├─────────────────────────────────────────────────────────────────┤
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ │
│  │ nexus-gate │ │ nexus-agent │ │  Redis      │ │  Prometheus │ │
